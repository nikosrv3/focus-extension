...
chrome.storage.local.get("BlockedUrls", (data) => {
    if (data.BlockedUrls !== undefined) {
        const isBlocked = data.BlockedUrls.some((e) => e.url === window.location.hostname && e.status === "BLOCKED");

        if (isBlocked) {
            CloseTab();
        } else {
            // Check if the page has already been reloaded after unblocking
            chrome.storage.local.get("ReloadedUrls", (reloadData) => {
                const reloadedUrls = reloadData.ReloadedUrls || [];
                if (!reloadedUrls.includes(window.location.hostname)) {
                    // Reload the page and mark it as reloaded
                    chrome.storage.local.set({
                        ReloadedUrls: [...reloadedUrls, window.location.hostname],
                    });
                    location.reload();
                }
            });
